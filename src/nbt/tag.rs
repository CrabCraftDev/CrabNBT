...
impl NbtTag {
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        bytes.extend(self.serialize_data().iter());
        bytes
    }
    ...
    pub fn serialize_data(&self) -> Vec<u8> {
        ...
        match self {
            NbtTag::End => {}
            NbtTag::Byte(byte) => bytes.extend(byte.to_be_bytes().iter()),
            NbtTag::Short(short) => bytes.extend(short.to_be_bytes().iter()),
            NbtTag::Int(int) => bytes.extend(int.to_be_bytes().iter()),
            NbtTag::Long(long) => bytes.extend(long.to_be_bytes().iter()),
            NbtTag::Float(float) => bytes.extend(float.to_be_bytes().iter()),
            NbtTag::Double(double) => bytes.extend(double.to_be_bytes().iter()),
            NbtTag::ByteArray(byte_array) => {
                bytes.extend((byte_array.len() as i32).to_be_bytes().iter());
                bytes.extend(byte_array);
            }
            NbtTag::String(string) => {
                let java_string = simd_cesu8::encode(string);
                bytes.extend((java_string.len() as u16).to_be_bytes().iter());
                bytes.extend_from_slice(&java_string[..]);
            }
            NbtTag::List(list) => {
                bytes.extend(
                    (list.first().unwrap_or(&NbtTag::End).get_type_id())
                        .to_be_bytes()
                        .iter(),
                );
                bytes.extend((list.len() as i32).to_be_bytes().iter());
                for nbt_tag in list {
            ...
            NbtTag::IntArray(int_array) => {
                bytes.extend((int_array.len() as i32).to_be_bytes().iter());
                for int in int_array {
                    bytes.extend(int.to_be_bytes().iter())
                }
            }
            NbtTag::LongArray(long_array) => {
                bytes.extend((long_array.len() as i32).to_be_bytes().iter());
                for long in long_array {
                    bytes.extend(long.to_be_bytes().iter())
                }
            }
        }
        bytes
